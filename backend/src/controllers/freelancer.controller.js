...
const registerFreelancer = async (req, res) => {
    try {
        ...
        const { fullName, email, password, number, skills, CNIC, bankAccount, currentlyEmployed, companyName, bio } = req.body;
...
const inactiveFreelancers = async (req, res) => {
    try {
        const freelancers = await Freelancer.find({ isActive: false }, 'fullName email');
...
const activeFreelancers = async (req, res) => {
    try {
        const freelancers = await Freelancer.find({ isActive: true }, 'fullName email');
        res.status(200).json(freelancers);
    } catch (err) {
        res.status(500).json({ error: 'Server error' });
    }
...
const toggleFreelancerStatus = async (req, res) => {
    try {
        ...
        // Only send a message instead of the entire object to reduce network traffic
        res.status(200).json({ message: 'Freelancer status updated.' });
    } catch (err) {
...
export { registerFreelancer, loginFreelancer, logoutFreelancer, getFreelancerProfile, inactiveFreelancers, activeFreelancers, toggleFreelancerStatus };
